...
public class PushBatchSender {
    ...
    public Map<String, Object> sendPushInBatches(List<Map<String, Object>> sendList) throws InterruptedException {
    	...
    	System.out.println("######sendPushInBatches START###########");
    	System.out.println("######총 전송 대상 수(sendList): " + sendList.size());
        ...
        resultMap.put("USER_LIST", allUserInfo);
        
        System.out.println("######sendPushInBatches END###########");
	...
	private BatchResult sendBatch(List<Map<String, Object>> batch) {
		...
		System.out.println("######sendBatch START###########");
		...
		for (Map<String, Object> sendMap : batch) {
			try {
				...
				String setYn = String.valueOf(sendMap.get("PUSH_NOTICE_SET_YN"));
				String noticeClf = String.valueOf(sendMap.get("NOTICE_CLF")); 
				...
				Message.Builder messageBuilder = Message.builder()
                        ...
                        .putData("badge", String.valueOf(badgeCnt))
                        ...
                        .putData("noticeClf", String.valueOf(sendMap.get("NOTICE_CLF")));				
				
				if(title != null && !title.isEmpty()) messageBuilder.putData("title", title);
				if(body != null && !body.isEmpty()) messageBuilder.putData("body", body);
				if(linkPage != null && !linkPage.isEmpty()) messageBuilder.putData("linkPage", linkPage);
                				
				Message message = messageBuilder.build();
				
				// JSON 디버깅용
				//Gson gson = new GsonBuilder().setPrettyPrinting().create();
				//System.out.println("=== FCM MESSAGE JSON ===");
		...
		try {
			...
			System.out.println("#################################################################");
			System.out.println("### sendBatch [ " + dateTimeFormat.format(new Date()) + " ] / send push start");
			for (int i = 0; i < messages.size(); i++) {
				...
				if (String.valueOf(sendMap.get("NOTICE_CLF")).equals("N")) {
					userInfo.put("sndSttus", "20");
					userInfoList.add(userInfo);
					continue;
				} else userInfo.put("sndSttus", "16");
				...
				try {
					...
					userInfo.put("sndSttus", "30");
					userInfo.put("errorMsg", e.getMessage());
					//System.out.println("FCM Fail result ===> [ " + i + " ] " + e.getMessage());
				}
				userInfoList.add(userInfo);
			}
			System.out.println("### sendBatch [ " + dateTimeFormat.format(new Date()) + " ] / send push finish");
			System.out.println("#################################################################");
		} catch (Exception e) {
			...
			for (Map<String, Object> sendMap : batch) {
				...
				if ("/page/sv/community.html".equals(sendMap.get("PUSH_LINK_PAGE"))) {
		...
		System.out.println("userInfoList 사이즈: " + userInfoList.size());
		System.out.println("######sendBatch END###########");
