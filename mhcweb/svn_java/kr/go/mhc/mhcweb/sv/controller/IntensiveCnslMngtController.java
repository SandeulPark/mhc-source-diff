...
public class IntensiveCnslMngtController extends DMultiActionController{
	...
	public String intensiveCnslMngtList(@ModelAttribute Map param, ModelMap model) throws Exception {
		...
		if(param.get("mainTotalYn") != null){
			...
			model.addAttribute("mainMclasCd",param.get("mainMclasCd"));
		}
	...
	public String intenseNutriObesityCnsl(@ModelAttribute Map param, ModelMap model) throws Exception {
		...
		model.addAttribute("rsOrgDtlsList", rsOrgDtlsList);// 2025.11
	 ...
	 * 상담 자동입력 팝업 화면 호출
	 */
	@RequestMapping(value= "/cnslAlgoAutoPop.do", method = RequestMethod.GET)
	public String cnslAlgoAutoPop(@ModelAttribute Map param, ModelMap model) throws Exception {
		return "web/sv/intenseCnslAlgoAuto";
	}
	 ...
	 * 상담 알고리즘 팝업 화면 호출
	 */
	@RequestMapping(value= "/cnslAlgoSetPop.do", method = RequestMethod.GET)
	public String cnslAlgoSetPop(@ModelAttribute Map param, ModelMap model) throws Exception {
		return "web/sv/intenseCnslAlgoSet";
	}	
	/**
	 * 상담 알고리즘  조건 
	 */
	@RequestMapping(value="/getAlgoPop.do",method = RequestMethod.POST)
	public @ResponseBody Map<String,Object> getAlgoPop(@ModelAttribute Map<String, Object> param, ModelMap model) throws Exception{
		Map<String,Object> rsMap = new HashMap<String,Object>();
		String searchInfo = StringUtil.nvl((String)param.get("SN"));
		if(!"".equals(searchInfo)){
			param.put("arr", StringUtil.makeStringToIterator(searchInfo));
		}
		List<Map<String,Object>> rsList = intensiveCnslMngtSerivce.getAlgoPop(param);
		rsMap.put("rsList",rsList);
	//	rsMap.put("id", param.get("id"));
		return rsMap;
	}
	 ...
	 * 자동 상담 알고리즘  조건 
	 */
	@RequestMapping(value = "/getAutoAlgoPop.do", method = RequestMethod.POST)
	public @ResponseBody Map<String, Object> getAutoAlgoPop(@ModelAttribute Map param, ModelMap model) throws Exception{
		Map<String,Object> rsMap = new HashMap<String,Object>();
		List<Map<String, Object>> rsList = intensiveCnslMngtSerivce.getAutoAlgoPop(param);
		rsMap.put("rsList", rsList);
		return rsMap;
	}
	 ...
	 * 자동 상담 알고리즘  조건 
	 */
	@RequestMapping(value = "/getAlgoPopSetting.do", method = RequestMethod.POST)
	public @ResponseBody Map<String, Object> getAlgoPopSetting(@ModelAttribute Map param, ModelMap model) throws Exception{
		Map<String,Object> rsMap = new HashMap<String,Object>();
		List<Map<String, Object>> rsList = intensiveCnslMngtSerivce.getAlgoPopSetting(param);
		rsMap.put("rsList", rsList);
