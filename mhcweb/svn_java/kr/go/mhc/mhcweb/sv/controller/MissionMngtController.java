...
import kr.go.mhc.mhcweb.sv.service.MissionMngtService;
...
public class MissionMngtController extends DMultiActionController {
	 ...
	 * @throws Exception 
	 */
	@RequestMapping(value="/sv/missionMngt.do", method= RequestMethod.GET)
	 ...
	 * @throws Exception 
	 */
	@RequestMapping(value= "/sv/missionMngtList.do", method= RequestMethod.POST)
	public @ResponseBody Map<String, Object> missionMngtList(@ModelAttribute Map<String, Object> param, ModelMap model) throws Exception {
		...
		if(param.get("pagingSet[gridRowsPerPage]") != null){
			int gridTotalRowCount = missionMngtService.getMissionMngtListCount(param);
			rsMap.put("gridTotalRowCount", gridTotalRowCount);
		}
		List<Map<String, String>> rsList = missionMngtService.getMissionMngtList(param);
	 ...
	 * @throws Exception
	 */
	@RequestMapping(value= "/sv/missionMngtDtls.do", method= RequestMethod.GET)
	public String missionMngtDtls(@ModelAttribute Map<String, Object> param, ModelMap model) throws Exception {
		...
		List<Map<String, String>> rsList = missionMngtService.getDtlsSelGrp(param);
		Map<String, String> rsMap = missionMngtService.getMissionMngtDtls(param);
		List<Map<String, String>> rsListGrp = missionMngtService.getDtlsSelGrp(param);
		//첨부파일 조회
		param.put("BOARD_SN", rsMap.get("BOARD_SN"));
		param.put("CMNTY_CD", rsMap.get("CMNTY_CD"));
		List<Map<String, String>> rsContAttch = missionMngtService.getSelectAttchFileList(param);

		if("11".equals(param.get("POST_CLF"))){
			//적용안된그룹 메인updateMission
			List<Map<String, String>> rsListPostGubun = missionMngtService.getNotSelMainGrp(param);
			model.addAttribute("rsListPostGubun", rsListPostGubun);
		} else if("12".equals(param.get("POST_CLF"))) {
			//적용안된그룹 커뮤니티
			List<Map<String, String>> rsListPostGubun = missionMngtService.getMissionPostGubun(param);
			model.addAttribute("rsListPostGubun", rsListPostGubun);
			List<Map<String, String>> rsListPostDtlsGubun = missionMngtService.getDtlsSelGrp(param);
			model.addAttribute("rsListPostDtlsGubun",rsListPostDtlsGubun);
		} 
		int rsReCnt = missionMngtService.getCmntyReCnt(param);
		int rsExpCnt = missionMngtService.getCmntyExpCnt(param);
...
//		 && "진행중".equals(param.get("MISSION_STTU_NM")) && ("0" == param.get("CMNTY_GRP_MB_CNT")))
		if("11".equals(param.get("POST_CLF"))){			
			return "web/sv/missionMngtMain";			
		} else {
			return "web/sv/missionMngtCmnty";
		} 
	 ...
	 * @throws Exception
	 */
	@RequestMapping(value= "/sv/updatePostYN.do", method= RequestMethod.POST)
	public @ResponseBody Map<String, Object> updatePostYN(@ModelAttribute Map<String, Object> param, ModelMap model) throws Exception {
		...
		Map<String, Object> rsMap = new HashMap<String, Object>();
		missionMngtService.updatePostYN(param);
	 ...
	 * @throws Exception
	 */
	@RequestMapping(value= "/sv/missionMngtReg.do", method= RequestMethod.GET)
	public String missionMngtReg(@ModelAttribute Map<String, Object> param, ModelMap model) throws Exception {
		List<Map<String, String>> rsList = missionMngtService.getMissionSelGubun(param);
		model.addAttribute("rsList",rsList);
		model.addAllAttributes(param);
		
		return "web/sv/missionMngtReg";
	}
	 ...
	 * @throws Exception
	 */
	@RequestMapping(value= "/sv/selMission.do", method= RequestMethod.POST)
	public @ResponseBody Map<String, String> selMssion(@ModelAttribute Map<String, Object> param, ModelMap model) throws Exception {
		Map<String, String> rsMap = missionMngtService.getSelMission(param);
		return rsMap;
	 ...
	 * @throws Exception
	 */
	@RequestMapping(value= "/sv/missionPostGubun.do", method= RequestMethod.GET)
	public @ResponseBody List<Map<String, String>> missionPostGubun(@ModelAttribute Map<String, Object> param, ModelMap model) throws Exception {
		List<Map<String, String>> rsList2 = missionMngtService.getMissionPostGubun(param);
		return rsList2;
	 ...
	 * @throws Exception
	 */
	@RequestMapping(value= "/sv/missionJoinInquire.do", method= RequestMethod.POST)
	public @ResponseBody Map<String, String> missionJoinInquire(@ModelAttribute Map<String, Object> param, ModelMap model) throws Exception {
		...
		if(grpIter.size() > 0){
			...
			rsMap = missionMngtService.getMissionJoinInquire(param);
		}
		return rsMap;
	 ...
	 * @throws Exception
	 */
	@RequestMapping(value= "/sv/insertMission.do", method= RequestMethod.POST)
	public void insertMission(@ModelAttribute Map<String, Object> param, ModelMap model, HttpServletRequest req) throws Exception {
		...
		int missionSN = missionMngtService.getMissionInquireSN(param);
		param.put("SN", missionSN);
		
		if("12".equals(param.get("POST_CLF"))){
			//커뮤니티 인 경우
			int selBoardSN = missionMngtService.getSelBoardSn(param);
			param.put("BOARD_SN", selBoardSN);
			missionMngtService.insertBoardGrp(param);
			missionMngtService.insertCmntyBoard(param);
		}
		
		missionMngtService.insertMission(param);		
		missionMngtService.insertMissionGrp(param);

		
		// 2. 푸시 전송 체크
		chkPushSnd(req,param);
		
//		return "redirect:../sv/missionMngt.do";
	 ...
	 * @throws Exception
	 */
	@RequestMapping(value= "/sv/updateMission.do", method= RequestMethod.POST)
	public void updateMission(@ModelAttribute Map<String, Object> param, ModelMap model, HttpServletRequest req) throws Exception {
		...
		if("12".equals(param.get("POST_CLF"))){
//		커뮤니티 인 경우
			missionMngtService.deletePostGrp(param);
			missionMngtService.insertBoardGrp(param);
			missionMngtService.updateCmntyBoard(param);
			missionMngtService.updateMissionScore(param);
			missionMngtService.updatePostYN(param);
		}
		
		missionMngtService.deleteMissionGrp(param);
		missionMngtService.insertMissionGrp(param);
		missionMngtService.updateMission(param);	
...
//		return "web/sv/missionMngt";
	 ...
	 * @throws Exception 
	 */
	@RequestMapping(value = "/sv/insertMissionScore.do", method = RequestMethod.POST)
	public String insertMissionScore(@ModelAttribute Map<String, Object> param, ModelMap model) throws Exception {
		missionMngtService.insertMissionScore(param);
		return "web/sv/missionMngtCmnty";
	}
	
	/**
	 * 미션 게시 시 푸시 메시지를 전송했는지 확인하고 미전송시 전송
	 * @param param 저장 정보
	 * @return 
	 * @throws Exception 
	 */
	private void chkPushSnd(HttpServletRequest req, Map<String, Object> param){
		...
		System.out.println("MissionMngtController / chkPushSnd param ====> " + param);
