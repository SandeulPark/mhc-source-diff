...
public class HealthMngtCnslController extends DMultiActionController {
	...
	public String healthMngtCnsl(@ModelAttribute Map param, ModelMap model) throws Exception {
		...
		// 복약미션
		if(StringUtil.nvl(rsMap.get("CHRONIC_DISEASES_YN"),"N").equalsIgnoreCase("Y")) {
			int drugCnt = healthMngtCnslService.selectDrugYn(param);
						
			if(drugCnt > 0) {
				model.addAttribute("DRUG_MISSION_YN", "Y");
			}
		}
	...
	public @ResponseBody void saveCnslContMngt(@ModelAttribute Map param, ModelMap model) throws Exception {		
		...
		if("N".equals(param.get("DRUG_MISSION_YN"))) {
			healthMngtCnslService.deleteDrugInfo(param);
		}
    }
	...
	public @ResponseBody Map<String, Object> schduleAfter(@ModelAttribute Map param, ModelMap model) throws Exception {
		...
		//만성질환 대상자일 경우 복약 미션 시작 일자 서비스 시작일로 업데이트  
		if(StringUtil.nvl(String.valueOf(param.get("CHRONIC_DISEASES_YN")),"N").equalsIgnoreCase("Y")) {
			healthMngtCnslService.updateDrugBgnDe(rsMap);
		}
...
//		param.put("SERVEY_TYPE", "20");
	 ...
	 * @throws Exception
	 */
	@RequestMapping(value ="/drugAddPop.do", method= RequestMethod.GET)
	public String drugPop(@ModelAttribute Map<String, Object> param, ModelMap model) throws Exception{
		
		List<Map<String, Object>> drugList = healthMngtCnslService.selectDrugInfo(param);
		
		model.addAttribute("SVC_MNGT_NO", param.get("SVC_MNGT_NO"));
		model.addAttribute("drugList",drugList);
		
		return "web/tg/drugAddPop";
	}
	 ...
	 * @throws Exception
	 */
	@RequestMapping(value = "/mergeDrugInfo.do", method = RequestMethod.POST)
	public @ResponseBody Map<String,Object> mergeDrugInfo(@ModelAttribute Map<String,Object> param, ModelMap model) throws Exception{
		String chkYn = "N";
		...
		try{	
			...
			//고혈압			
			rsMap.put("DRUG_CD", "D10");
			rsMap.put("DRUG_TIME", param.get("hbpTime"));
			rsMap.put("PUSH_SET_USE_YN", param.get("hbpPushUseYn"));
			rsMap.put("USE_YN", param.get("hbpUseYn"));
			healthMngtCnslService.mergeDrugInfo(rsMap);
			
			//당뇨병
			rsMap.put("DRUG_CD", "D20");
			rsMap.put("DRUG_TIME", param.get("diaTime"));
			rsMap.put("PUSH_SET_USE_YN", param.get("diaPushUseYn"));
			rsMap.put("USE_YN", param.get("diaUseYn"));	
			healthMngtCnslService.mergeDrugInfo(rsMap);
			
			//이상지질혈증
			rsMap.put("DRUG_CD", "D30");
			rsMap.put("DRUG_TIME", param.get("dysTime"));
			rsMap.put("PUSH_SET_USE_YN", param.get("dysPushUseYn"));
			rsMap.put("USE_YN", param.get("dysUseYn"));
			healthMngtCnslService.mergeDrugInfo(rsMap);		
	 ...
	 * @throws Exception
	 */
	@RequestMapping(value ="/drugSaveYn.do", method = RequestMethod.POST)
	public Map<String, Object> drugSaveYn(@ModelAttribute Map<String, Object> param, ModelMap model) throws Exception{
		
		Map<String,Object> rsMap = new HashMap<String, Object>();		
		String drugSaveYn = "N";
		if(healthMngtCnslService.selectDrugYn(param) > 0) drugSaveYn = "Y";				
		rsMap.put("drugSaveYn", drugSaveYn);		
		return rsMap;
	}
}
