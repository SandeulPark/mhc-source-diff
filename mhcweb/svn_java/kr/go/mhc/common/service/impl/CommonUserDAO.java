...
public class CommonUserDAO extends DMultiEgovAbstractMapper{
	...
	public Map<String, Object> checkLoginUser(Map<String, Object> param)
		...
		if(mainMap != null){
			...
			if(!"Y".equals(StringUtil.nvl(mainMap.get("APPROVAL_YN")))){
				...
				bLogin	= false;
			}else if(!"Y".equals(StringUtil.nvl(mainMap.get("DORMANT_YN")))){
				msgCode = "common.login.dormantYn";
				bLogin	= false;
			}
	...
	public Map<String, Object> checkWebLoginUser(Map<String, Object> param)
			throws Exception {
		// TODO Auto-generated method stub
		...
		String				loginGb		= String.valueOf(param.get("loginGb"));
		...
		System.out.println("############loginGb ===> " + loginGb);
		
		if(!sttusList.isEmpty()){
			if(!"Y".equals(StringUtil.nvl(sttusList.get(0).get("ORG_APPROVAL_YN")))){
				msgCode = "common.login.noApproval";
				bLogin	= false;				
			}
			if(!"Y".equals(StringUtil.nvl(sttusList.get(0).get("APPROVAL_YN")))){
				msgCode = "common.login.noApproval";
				bLogin	= false;
			}else if(!"Y".equals(StringUtil.nvl(sttusList.get(0).get("DORMANT_YN")))){
				msgCode = "common.login.dormantYn";
				bLogin	= false;
			}
			...
			}else if("loginVpn".equals(loginGb)) { // vpn로그인 시
				System.out.println("loginVpn~~~");
			...
			}else if("loginVpn".equals(loginGb)){
				msgCode = "common.login.chkIdPw";
			}else{									//기본 로그인할 경우
	...
	public Map<String, Object> checkSessionUser(Map<String, Object> param) {
		Map<String, Object> rtMap		= new HashMap<String,Object>();
		Map<String, String>	rsMap		= null;
		rsMap = selectOne("common.user.checkSessionUser", param);
		rtMap.put("rsMap", rsMap);
