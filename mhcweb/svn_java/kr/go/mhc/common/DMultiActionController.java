...
import kr.go.mhc.common.util.PaginationUtil;
...
public abstract class DMultiActionController{
		...
		if(req.getRequestURI().indexOf(".do") > 0 && "/login/loginPage.do|/cmmn/selectCmmnMenu.do|/cmmn/selectCmmnCd.do|/login/loginPageOld.do".indexOf(req.getRequestURI()) < 0){
			...
			try {
				...
				//2019.10.16 오샘이 로그 저장시 CLOB데이터 파람으로 넘어로경우애러나서 기존 3900을 1500자까지만 자르게 수정 
				 // param.put("REQ_PARAM", sb.toString().length() > 3800 ? sb.toString().substring(0, 1500) : sb.toString());

				// 2024.04.15 로그 저장시, CLOB 데이터 파람으로 넘어올 경우 에러 발생 > 기존 string length 를 byte length 로 변경
				param.put("REQ_PARAM", sbBytes.length > 3900 ? sb.toString().substring(0, 1500) : sb.toString());
				
				System.out.println("#################### DMulti ### 0> " + sb.toString().length() );
				System.out.println("#################### DMulti ### 1> " + sbBytes.length );
	...
	public Map<String,Object> getSessionInfo(HttpServletRequest req){
		...
		if(!"".equals(userId)){
			...
			//담당업무명 추가
			sessMap.put("SESS_JOB_CLF_NM",	StringUtil.nvl(String.valueOf(session.getAttribute("JOB_CLF_NM"))));

			// 만성질환 보건소인가요?
			sessMap.put("SESS_CHRONIC_ORG",	String.valueOf(session.getAttribute("SESS_CHRONIC_ORG")));

			// 자가관리군 집중상담/중간검진 진행 여부
			sessMap.put("SESS_INTENS_CNSL_USE_YN", String.valueOf(session.getAttribute("SESS_INTENS_CNSL_USE_YN")));
			sessMap.put("SESS_MID_EXAM_USE_YN",	String.valueOf(session.getAttribute("SESS_MID_EXAM_USE_YN")));
		}
		return sessMap;
	}
	...
	public void setSessionInfo(HttpServletRequest req, HttpSession session, Map<String, String> param,
		...
		if (!"".equals(param.get("USER_ID"))) {
			...
			for (int i = 0; i < param2.size(); i++) {
				session.setAttribute("AUTH_MENU" + i, param2.get(i));
			}

			session.setAttribute("AUTH_MENU", param2);//권한 정보를 담음
			loginManager.printloginUsers();
		}
	}
