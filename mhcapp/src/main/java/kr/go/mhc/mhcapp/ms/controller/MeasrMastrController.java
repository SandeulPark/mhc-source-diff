...
public class MeasrMastrController extends DMultiActionController{ 
	 ...
	 * @throws Exception 
	 */
	@RequestMapping( value="/updateBloodSugarMealClf.do" ,method=RequestMethod.POST)
	public @ResponseBody int updateBloodSugarMealClf(HttpServletRequest req, @ModelAttribute Map<String,Object> param, ModelMap model){
		int r = 0;
		try {
			r = measrMastrService.updateBloodSugarMealClf(param);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return r;
	...
	public @ResponseBody Map<String,Object> insertMeasrData(@ModelAttribute Map<String,Object> param, ModelMap model){
		...
		try{
			...
			if ("TN_MS_ACT".equals(tblNm)) {
				param.put("MEASR_TRGT_CLF", "10"); 	//측정대상구분_활동량 : MS001_10
