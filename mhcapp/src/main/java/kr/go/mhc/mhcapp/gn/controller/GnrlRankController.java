...
@Controller
@RequestMapping( value="/gn/rank")
public class GnrlRankController extends DMultiActionController{

	@Resource(name="gn.gnrlRankService")
	private GnrlRankService gnrlRankService;
	...
	public @ResponseBody Map<String,Object> selectRankList(@ModelAttribute Map<String,Object> param, ModelMap model) throws Exception{
		...
		try{			
			List<Map<String,String>> orgList = gnrlRankService.selectRankInfo(param); //보건소
			param.put("SCH_TYPE", "");
			List<Map<String,String>> rankList = gnrlRankService.selectRankInfo(param); //내점수
	...
	public @ResponseBody Map<String,Object> selectRankMyScoreList(@ModelAttribute Map<String,Object> param, ModelMap model) throws Exception{
		...
		try{
			List<Map<String,String>> rsList1 = gnrlRankService.selectRankMyScoreList1(param);   
			List<Map<String,String>> rsList2 = gnrlRankService.selectRankMyScoreList2(param);   
		...
		try{
			if(param.containsKey("RANK_CATEGORY")){
				...
				if("1".equals(category)){
					...
					rsList = gnrlRankService.selectRankDtlsList(param);   //전국 순위목록
				}else if("2".equals(category)){
					param.put("SCH_TYPE", "GENDER");
					rsList = gnrlRankService.selectRankDtlsList(param);   //성별 순위목록
				}else if("3".equals(category)){
					param.put("SCH_TYPE", "AGE");
					rsList = gnrlRankService.selectRankDtlsList(param);   //연령 순위목록
				}else if("4".equals(category)){
					param.put("SCH_TYPE", "ORG");
					rsList = gnrlRankService.selectRankDtlsList(param);   //보건소 순위목록
				}
			}
			myList = gnrlRankService.selectRankDtls(param); // 내순위
	...
	public @ResponseBody Map<String,Object> selectPointList(@ModelAttribute Map<String,Object> param, ModelMap model) throws Exception{
		...
		try{
			List<Map<String,String>> rsList1 = gnrlRankService.selectRankList(param); //내점수
			List<Map<String,String>> rsList2 = gnrlRankService.selectPointList(param); //포인트 이력
	...
	public @ResponseBody Map<String,Object> actRankList(@ModelAttribute Map<String,Object> param, ModelMap model) throws Exception{
		...
		try{			
			List<Map<String,String>> rankList = gnrlRankService.selectActRankInfo(param); //전체
			rsMap_sb.put("rankList", rankList);
	...
	public @ResponseBody Map<String,Object> actRankDtlsList(@ModelAttribute Map<String,Object> param, ModelMap model) throws Exception{
		...
		try{
			...
			if(param.containsKey("RANK_CATEGORY")){ // 내순위
				...
				if("1".equals(category)){
					param.put("SCH_TYPE", "ALL");
					rsList = gnrlRankService.selectActRankList(param);   //전국 순위목록
				}else if("2".equals(category)){
					param.put("SCH_TYPE", "GENDER");
					rsList = gnrlRankService.selectActRankList(param);   //성별 순위목록
				}else if("3".equals(category)){
					param.put("SCH_TYPE", "AGE");
					rsList = gnrlRankService.selectActRankList(param);   //연령 순위목록
				}else if("4".equals(category)){
					param.put("SCH_TYPE", "ORG");
					rsList = gnrlRankService.selectActRankList(param);   //보건소 순위목록
				}
			}
			myList = gnrlRankService.selectActRankStatData(param);
			rsMap_sb.put("rsList", rsList);
			rsMap_sb.put("myList", myList);
			chkYn = "Y";			
		}catch(Exception e){
	...
	public @ResponseBody Map<String,Object> actRankChartData(@ModelAttribute Map<String,Object> param, ModelMap model) throws Exception{
		...
		try{
			rsList = gnrlRankService.selectActRankChartInfo(param);
			rsMap_sb.put("rsList", rsList);
			chkYn = "Y";			
		}catch(Exception e){
			e.printStackTrace();
	...
	public @ResponseBody Map<String,Object> realTimePointPop(@ModelAttribute Map<String,Object> param, ModelMap model) throws Exception{
		...
		try{
			...
			// 랭킹 월 조회
			rsList = gnrlRankService.selectRankYm(param);
			param.putAll(rsList.get(0));
			
			// 랭킹 주 조회
			rsList = gnrlRankService.selectRankWeek(param);
			param.putAll(rsList.get(0));
			...
			rsList = gnrlRankService.selectMissionChk(param);
			regCnt = Integer.parseInt(String.valueOf(rsList.get(0).get("REG_CNT")));
			
			// 미션 달성 하지 않은 경우
			if(regCnt == 0 || "0042".equals(missionCd)){				
				// 미션 점수 조회
				rsList = gnrlRankService.selectMissionScore(param);
				param.putAll(rsList.get(0));
				
				// 랭킹 달성 점수 조회
				rsList = gnrlRankService.selectMissionAchvScore(param);
				param.putAll(rsList.get(0));
				...
				if(achvScore < maxScore){
					...
					if("0027".equals(missionCd)){
						int measrDeCnt = 0;
						rsList = gnrlRankService.selectMission_00270028(param);
						
						if(rsList.size() > 0){
							measrDeCnt = Integer.parseInt(String.valueOf(rsList.get(0).get("MEASR_DE_CNT")));
							
							if(measrDeCnt >= 5){
								gnrlRankService.insertMissionAchvDta(param);
								insertFlag = true;
							}
						}
					}
					...
					/*
						...
						int measrDeCnt = 0;
						rsList = gnrlRankService.selectMission_0029(param);
					...
					*/
					// 일일 목표걸음수 달성(활동량계 및 스마트폰 자체 걸음수 둘 중 하나라도 달성 시)
						...
						int totActCnt = 0;
						int walkCnt = 0;
						rsList = gnrlRankService.selectMission_0030(param);
					...
					// 주간 식생활미션 주5일 이상 확인
						...
						int answrDeCnt = 0;
						rsList = gnrlRankService.selectMission_00310032(param);
					...
					/*
						...
						int answrDeCnt = 0;
						rsList = gnrlRankService.selectMission_0033(param);
					...
					*/
					...
					// 식사기록 [가산] 주7일 모두 확인
						...
						int mealDeCnt = 0;
						rsList = gnrlRankService.selectMission_0038(param);
					...
					// 게시판 덧글달기(단, 게시물 당 1회)
						...
						rsList = gnrlRankService.selectMission_0040(param);
					...
					// 게시판 감정표현(단, 게시물 당 1회)
						...
						rsList = gnrlRankService.selectMission_0041(param);
					...
					// 콘텐츠 내용 SNS 공유하기
						...
						rsList = gnrlRankService.selectMission_0042(param);
	...
	public @ResponseBody Map<String,Object> badgeAchvSttus(@ModelAttribute Map<String,Object> param, ModelMap model) throws Exception{
		...
		try{			
			badgeMap  = gnrlRankService.selectBadgeAchvSttus(param); //전체
			chkYn = "Y";
		}catch(Exception e){
			e.printStackTrace();
		}
	...
	public @ResponseBody Map<String,Object> badgeAchvDtlsList(@ModelAttribute Map<String,Object> param, ModelMap model) throws Exception{
		...
		try{			
			rankList = gnrlRankService.selectBadgeAchvDtlsList(param); 
			chkYn = "Y";
		}catch(Exception e){
			e.printStackTrace();
		}
