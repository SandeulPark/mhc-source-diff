...
//        20140703 추가 
    ...
    */
	...
	public static List<Map<String,String>> makeStringToIterator(String strParam){
		return makeStringToIterator(strParam,null,null,null);
	}
	 ...
	 * @return List<Map<String,String>> 
	 */
	public static List<Map<String,String>> makeStringToIterator(String strParam, String rowToken, String colToken, String valToken){
		List<Map<String,String>> rtnList = null;//new ArrayList<Map<String,String>>();
		
		if(rowToken == null) rowToken = ",";
		if(colToken == null) colToken = "^";
		if(valToken == null) valToken = "=";
		
		if(!"".equals(nvl(strParam))){
			try {
				if(strParam.indexOf("&quot;") > 0) strParam = strParam.replace("[{","").replace("}]","").replaceAll("&quot;", "");
				...
				while (arrRow.hasMoreTokens()) {
					...
					if(!"".equals(strCol)){
						...
						for (int col = 0; arrCol.hasMoreTokens(); col++) {
							...
							if(!"".equals(strVal)){
								strVal = strVal.replaceAll("@amp;", "&");
								if(strVal.indexOf(valToken) > 0){
									dataMap.put(strVal.substring(0,strVal.indexOf(valToken)), strVal.substring(strVal.indexOf(valToken)+1));
//								}else if(strVal.indexOf(":") > 0){
//									dataMap.put(strVal.substring(0,strVal.indexOf(":")), strVal.substring(strVal.indexOf(":")+1));
								}else{
